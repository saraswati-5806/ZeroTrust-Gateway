...
# Fallback client if watson_client was removed
try:
    from ai.watsonx_client import watson_client
except ImportError:
    class MockWatsonClient:
        def generate_explanation(self, context):
            return f"Risk evaluated at score {context.get('risk_score', 0)}. Action: {context.get('decision', 'ALLOW')}."
    watson_client = MockWatsonClient()

def evaluate_risk(context):
    """
    Evaluates incoming request context through the Isolation Forest model 
    and generates a human-readable threat explanation.
    """
    feature_payload = {
