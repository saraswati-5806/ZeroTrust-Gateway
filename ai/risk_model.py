    ...
    def predict_risk(self, features_dict):
        ...
        try:
            feature_vector = np.array([[
                float(features_dict.get('login_hour', 10)),
                float(features_dict.get('failed_attempts', 0)),
                float(features_dict.get('geo_distance_km', 0.0)),
                float(features_dict.get('device_known', 1)),
                float(features_dict.get('role_mismatch', 0))
            ]])
            ...
            # Fallback calculation if model transformation fails
            return 50
